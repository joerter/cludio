...
(defn header
  [year month next-month-path prev-month-path]
  [:header {:class "flex items-center justify-between border-b border-gray-200 px-6 py-4 lg:flex-none"}
   [:h1 {:class "text-base font-semibold leading-6 text-gray-900"}
    [:time {:datetime "2022-01"} (str month " " year)]]
   [:div {:class "flex items-center"}
    (month-selector month next-month-path prev-month-path)
    (view-picker)]])
...
(defn month-day [{:keys [current-month? today? local-date classes]}]
  (let [current-month "bg-white"
    ...
    [:div {:class (str (if current-month? current-month other-month) " relative px-3 py-2")}
     [:time {:datetime (jt/format "yyyy-MM-dd" local-date) :class (if today? today "")} (jt/as local-date :day-of-month)]
     (when (seq classes) (into [:ol {:class "mt-2"}]
                               (map (fn [{:keys [datetime name]}]
                                      [:li [:a {:href "#" :class "group flex"}
                                            [:p {:class "flex-auto truncate font-medium text-gray-900 group-hover:text-indigo-600"} name]
                                            [:time {:datetime "2023-11-11" :class "ml-3 hidden flex-none text-gray-500 group-hover:text-indigo-600 xl:block"} (jt/format "h a" datetime)]]]) classes)))]))
